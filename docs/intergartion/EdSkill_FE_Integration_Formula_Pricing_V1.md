...
Delta moi cho companion discovery/detail online-only va bo loc duration/price/credential:

- `docs/intergartion/EdSkill_FE_Update_Companion_Search_Online_Only.md`

Delta moi cho create-session online-only va skill ownership:
...
| `durationOptions` | Source chinh de render dropdown/radio chon thoi luong tren formula session. Rule moi: BE luon mo rong danh sach theo moc lon nhat. Vi du session 120 phut se tra `[30,45,60,90,120]`; session 90 phut se tra `[30,45,60,90]`. Neu list rong va `pricingModel = "LegacyManual"` thi FE bo qua. |
...
| `iconKey` | FE map sang icon component trong icon library. Neu `null` thi render fallback hoac khong hien icon. |
...
| `iconKey` | `string \| null` | Optional | neu gui len: trim, max 50 ky tu, regex `^[a-z0-9]+(?:-[a-z0-9]+)*$` | `"book-open"` |
...
| `iconKey` | `string \| null` | Optional | neu gui len: `null`/`""`/whitespace -> xoa icon; con lai max 50, regex `^[a-z0-9]+(?:-[a-z0-9]+)*$` | `"paintbrush"` |
...
| `pricingPreview` | Dung de hien thi range gia neu session formula co nhieu duration. Neu `minLearnerChargePoints != maxLearnerChargePoints`, render dang `169 - 319 points`. Range nay phan anh toan bo cac moc learner co the chon trong `durationOptions` ma BE da mo rong. |
...
- Goi `on submit`.
- Session create flow nay da la `online-only`.
- FE phai lay skill dropdown tu `GET /api/profile/me -> teachingSkills`.
- FE khong cho user chon hinh thuc hoc va khong gui `deliveryMode`/`location`.
...
| Field | Type | Required | Validate | Vi du |
| --- | --- | --- | --- | --- |
| `skillId` | `guid` | Required | GUID hop le va phai nam trong `teachingSkills` cua companion | `"c3b9a31c-123d-4c0f-a821-2b84613b7fe9"` |
| `description` | `string \| null` | Optional | max 2000 | `"Luyen giao tiep co ban"` |
| `durationOptions` | `number[]` | Required | khong rong, chi duoc chon trong `[30,45,60,90,120]`. FE moi nen gui 1 phan tu duy nhat la moc lon nhat companion mo. BE se tu mo rong cac moc nho hon hop le. Vi du gui `[120]` -> response session co `durationOptions = [30,45,60,90,120]`; gui `[90]` -> response co `[30,45,60,90]` | `[120]` |
...
- FE can dung:
  - `durationOptions` de render lai list duration trong preview/create success. List nay da duoc BE mo rong.
...
- `400 INVALID_DURATION_OPTIONS`:
  - Inline error field duration: `Chi duoc chon moc 30, 45, 60, 90 hoac 120 phut`.
...
- FE moi nen coi `durationOptions` trong request la `moc lon nhat companion mo`, khong phai danh sach custom learner durations.
- FE KHONG gui:
  - `deliveryMode`
  - `location`
  - `pointCost`
...
- BE tu tinh pricing preview dua tren:
  ...
  - moc lon nhat trong `durationOptions`, sau do BE tu mo rong thanh cac moc nho hon hop le
  - system markup 25%
...
- Voi formula session chua book:
  - `durationOptions` moi la source render option, va list nay da duoc BE mo rong theo moc lon nhat
...
| Field | Type | Required | Validate | Vi du |
| --- | --- | --- | --- | --- |
| `selectedDurationMinutes` | `number` | Required | phai nam trong `[30,45,60,90,120]`, va quan trong hon: phai thuoc `durationOptions` session tra ve. Vi du offer 120 phut thi co the gui `30`, `45`, `60`, `90` hoac `120` | `30` |
...
- Voi session formula cu trong DB chi luu moc lon nhat, BE van tu mo rong luc doc/book.
- Voi legacy session ton tai tu du lieu cu:
  - endpoint van hoat dong
  - `selectedDurationMinutes` van phai gui hop le theo validator; FE moi scope formula nen chi show duration picker khi `pricingModel = "FormulaV1"`
...
```json
{
  ...
  "pricingPreview": {
    "minCompanionPayoutPoints": 135,
    "maxCompanionPayoutPoints": 135,
    "minLearnerChargePoints": 169,
    "maxLearnerChargePoints": 169,
    "minPlatformFeePoints": 34,
    "maxPlatformFeePoints": 34
  },
  "pricingBreakdown": {
...
1. Form create session:
   ...
   - UI companion nen cho chon 1 moc lon nhat: `30` hoac `45` hoac `60` hoac `90` hoac `120`.
   - FE gui moc lon nhat trong `durationOptions`, vi du `[120]`.

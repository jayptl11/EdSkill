...
Signup intent hop le cho public register:
...
- FE khong cho user chon role thu cong trong form register.
- FE map theo CTA truoc dang nhap:
  - `Hoc ngay` -> `signupIntent = "learn"`
  - `Day hoc` -> `signupIntent = "teach"`
- Backend map role noi bo:
  - `learn` -> `["learner"]`
...
Gui thong tin dang ky va `signupIntent`. Backend se gui OTP qua email. User chi duoc tao sau khi verify OTP thanh cong.
...
Google login nhan `signupIntent` cho truong hop tao account moi.
...
## 4.1. Redirect sau auth theo CTA

- Neu user di tu CTA `Day hoc`:
  - dang ky/login voi `signupIntent = "teach"`
  - sau verify/login thanh cong, FE redirect vao companion onboarding wizard
- Neu user di tu CTA `Hoc ngay`:
  - dang ky/login voi `signupIntent = "learn"`
  - sau verify/login thanh cong, vao learner flow mac dinh
- Learner cu bam `Day hoc`:
  - goi `POST /api/profile/me/enable-companion`
  - sau do redirect vao cung companion onboarding wizard

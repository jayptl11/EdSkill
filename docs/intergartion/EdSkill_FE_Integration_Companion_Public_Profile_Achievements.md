...
- Backend cung khong nhan `multipart/form-data` trong `POST /api/admin/achievements/icon-upload-url`
- Endpoint `icon-upload-url` chi nhan JSON metadata cua file
...
### Troubleshooting `422` khi upload icon

Neu FE chon anh tu may va bi `422`, kiem tra 4 diem nay truoc:

1. FE co dang goi `icon-upload-url` bang JSON khong.
2. `contentType` co dung 1 trong cac gia tri duoc ho tro khong:
   - `image/jpeg`
   - `image/jpg`
   - `image/png`
   - `image/webp`
3. `fileSize` co nho hon hoac bang `10MB` khong.
4. `fileName` co duoc gui khong.
